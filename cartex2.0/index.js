...
app.get('/api/cartes/A-Z', async (req, res) => {
    try {
        const conn = await pool.getConnection();
        const results = await conn.query('SELECT * FROM cartes ORDER BY name ASC;');
        conn.release();
        res.json(results);
    } catch (error) {
        console.error('Error fetching data from database: ' + error.message);
...
app.get('/api/cartes/filtreprix', async (req, res) => {
    try {
        const conn = await pool.getConnection();
        const results = await conn.query('SELECT * FROM cartes ORDER BY cards_price ASC;');
        conn.release();
        res.json(results);
    } catch (error) {
        console.error('Error fetching data from database: ' + error.message);
...
app.get('/api/cartes/type', async (req, res) => {
    try {
        const conn = await pool.getConnection();
        // Ajoutez un WHERE pour filtrer par type
        const results = await conn.query("SELECT * FROM cartes WHERE type = 'Effect Monster' ORDER BY type ASC;");
        conn.release();
        res.json(results);
    } catch (error) {
        console.error('Error fetching data from database: ' + error.message);
